...
");
            ...
            foreach (var r in relations)
            {
                ...
                builder.AppendLine();
                if (r.Relation == Core.DbConfig.RelationType.ManyToMany)
                {
                    var f_r = conn.FindRelations(r.ForeignTable, r.MainTable)[0];
                    builder.AppendFormat("  .HasMany(x => x.{0}) ", r.NavigateProperty);
                    builder.AppendLine();
                    builder.AppendFormat("  .WithMany(x => y.{0}) ", r.NavigateProperty);
                    builder.AppendLine();
                    builder.AppendFormat("  .Map(m =>  ", r.NavigateProperty);
                    builder.AppendLine();
                    builder.Append(" \t{");
                    builder.AppendLine();
                    builder.AppendFormat(" \t\tm.ToTable(\"{0}\");  ", r.ManyToManyTable);
                    builder.AppendLine();
                    builder.AppendFormat(" \t\tm.MapLeftKey(\"{0}\");  ", r.MainFK);
                    builder.AppendLine();
                    builder.AppendFormat(" \t\tm.MapRightKey(\"{0}\");  ", f_r.MainFK);
                    builder.AppendLine();
                    builder.Append(" \t});");
                }
                else
