...
");
    ...
    :base(id)", this.Model.Name, idColumn.SystemType.ToString()));
            ...
            foreach (FieldType field in this.Model.Fields.OrderByName())
            {
                ...
                if (keyColumn != null && (field.Name == keyColumn.Name && keyColumn.Name == "Id"))
                {                    
                    continue;
                }
                else if (keyColumn != null && (field.Name == keyColumn.Name)){
                    builder.AppendLine("/// <summary>");
                    builder.AppendLine("///" + field.Description);
                    builder.AppendLine("/// </summary>");
                    builder.AppendLine(" [NotMapped]");
                    builder.AppendFormat(" public override {0} Id",field.SystemType);
                    builder.AppendLine();
                    builder.AppendLine("{");
                    ...
                    builder.AppendLine(" }");
                    builder.AppendLine(" protected set{}");                    
                    builder.AppendLine(" }");
                }
            ...
            {                
                ...
                else if (r.Relation == Core.DbConfig.RelationType.ManyToMany)
                {
                    builder.AppendFormat("public virtual List<{0}> {1}", fkModel.Name, Helper.GetClassName(r.NavigateProperty));
                    builder.AppendLine();
                    builder.AppendLine("{get;set;}");
                }
                else if (r.Relation == Core.DbConfig.RelationType.ManyToOne)
                {
                    builder.AppendFormat("public virtual {0} {1}", fkModel.Name, Helper.GetClassName(r.NavigateProperty));
                    builder.AppendLine();
                    builder.AppendLine("{get;set;}");
                }
                else if (r.Relation == Core.DbConfig.RelationType.OneToMany)
                {
                    builder.AppendFormat("public virtual List<{0}> {1}", fkModel.Name, Helper.GetClassName(r.NavigateProperty));
                    builder.AppendLine();
                    builder.AppendLine("{get;set;}");
