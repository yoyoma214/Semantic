...
using CodeHelper.DataBase.GenerateUnit.GenSchema;
...
namespace CodeHelper.DataBaseHelper.Items.DBItems
{
    class ConnectionNode : BaseNode
    {
        ...
        {
            var conn = ConnectionManager.Get(this.ConncetionString);
            conn.ConnData = this.dataInfo;
            conn.Name = this.dataInfo.Name;
            var gen = new GenSchema(conn);
        ...
        private void CompareOtherConnection_Click(object sender, EventArgs args)
        {
            try
            {
                ...
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (otherConn.DbType != this.dataInfo.DbType)
                    {
                        MessageBox.Show("两个连接串对应的数据库类型不一致，不能比较！");
                        return;
                    }
        ...
        private void CompareViewCode(ConnectionType otherConn, List<TableType> thisObjs, List<TableType> otherObjs, StringBuilder builder)
        {
            ...
            {
                CompareViewCode_Sql(otherConn, thisObjs, otherObjs, builder);
            }
            else if (this.DbType == DatabaseType.MySql)
            {
                CompareViewCode_MySql(otherConn, thisObjs, otherObjs, builder);
            }
        }

        private void CompareViewCode_Sql(ConnectionType otherConn, List<TableType> thisObjs, List<TableType> otherObjs, StringBuilder builder)
        {
            try
            {
...
and obj.name in ({0}) order by obj.name", v_str);
        ...
        private void CompareViewCode_MySql(ConnectionType otherConnection, List<TableType> thisObjs, List<TableType> otherObjs, StringBuilder builder)
        {
            try
            {
                var thisConn = new MySqlConnection(this.ConncetionString);
                var otherConn = new MySqlConnection(otherConnection.ConnectionString);
                var views = new List<string>();
                foreach (var thisView in thisObjs)
                {
                    if (otherObjs.FindTable(thisView.Name) != null)
                ...
                var sql = string.Format(@"select * from information_schema.views where  table_schema = 'test'
order by table_name", v_str);

                var thisDs = MySqlHelper.GetDataSet(thisConn, CommandType.Text, sql, null);

                var otherDs = MySqlHelper.GetDataSet(otherConn, CommandType.Text, sql, null);
                ...
                for (var index = 0; index < thisDs.Tables[0].Rows.Count; index++)
                {
                    ...
                    var thisSql = thisRow["VIEW_DEFINITION"].ToString().Trim();
                    var otherSql = otherRow["VIEW_DEFINITION"].ToString().Trim();
                    ...
                    if (thisFirstLine != otherFirstLine)
                    {
                        builder.AppendFormat("View Difference {0}", thisRow["TABLE_NAME"].ToString());
                        builder.AppendLine();
                        continue;
                    }
        ...
        private void CompareFunctionCode(ConnectionType otherConn,StringBuilder builder)
        {
            if (this.DbType == DatabaseType.SqlServer)
            {
                CompareFunctionCode_Sql(otherConn, builder);
            }
            else if (this.DbType == DatabaseType.SqlServer)
            {
                CompareFunctionCode_MySql(otherConn, builder);
            }
        }

        private void CompareFunctionCode_Sql(ConnectionType otherConn, StringBuilder builder)
        {
            var helper = new DbHelper(this.dataInfo);
            var otherHelper = new DbHelper(otherConn);
            var views = new List<string>();
...
order by obj.name", v_str);
        ...
        private void CompareFunctionCode_MySql(ConnectionType otherConnection, StringBuilder builder)
        {
            var thisConn = new MySqlConnection(this.ConncetionString);
            var otherConn = new MySqlConnection(otherConnection.ConnectionString);
            var views = new List<string>();
            ...
            var sql = string.Format(@"show function status");
            ...
            for (var index = 0; index < thisDs.Tables[0].Rows.Count; index++)
            {
                var thisRow = thisDs.Tables[0].Rows[index];
                var this_name = thisRow["name"].ToString().Trim();
                //var def_sql = string.Format("show create function {0}",this_name);
                //var def_this = MySqlHelper.GetDataSet(thisConn, CommandType.Text, def_sql, null);
                //var this_definition = thisRow["definition"].ToString().Trim();
                ...
                if (!exsited)
                {
                    builder.AppendFormat("本地新增： {0}",this_name);
                    builder.AppendLine();
                }
            }
            #endregion
            ...
            for (var index = 0; index < otherDs.Tables[0].Rows.Count; index++)
            {
                ...
                if (!exsited)
                {
                    builder.AppendFormat("本地缺少： {0}", other_name);
                    builder.AppendLine();
                }
            }
            #endregion
            ...
            for (var index = 0; index < thisDs.Tables[0].Rows.Count; index++)
            {
                ...
                //var this_type = thisRow["type"].ToString().Trim();
                var this_definition = thisRow["Create Function"].ToString().Trim();
                var this_name = thisRow["name"].ToString().Trim();                

                for (var j = 0; j < otherDs.Tables[0].Rows.Count; j++)
                {
                    var otherRow = otherDs.Tables[0].Rows[j];                    
                    var other_definition = otherRow["Create Function"].ToString().Trim();
                    var other_name = otherRow["name"].ToString().Trim();
                    ...
                    if (this_name == other_name)
                    {
                        if (this_definition != other_definition)
                        {
                            builder.AppendFormat("函数内容不同： {0}", this_name);
                            builder.AppendLine();
                        }
                        continue;
                    }
