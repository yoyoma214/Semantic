    ...
    partial class ProjectPanel: IReceiverHolder
    {
        ...
        bool receiver_OnNewNullProject()
        {
            ...
            if (dlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                root.DataInfo = GlobalService.CurrentProject;
        ...
        private void OpenPrject()
        {
            ...
            NodeFactory.Create(Dict.NodeType.Swrl).Parent = root;
            //NodeFactory.Create(Dict.NodeType.XmlModelSet).Parent = root;
            //NodeFactory.Create(Dict.NodeType.DataModelSet).Parent = root;
            //NodeFactory.Create(Dict.NodeType.DataViewSet).Parent = root;
            //NodeFactory.Create(Dict.NodeType.ViewModelSet).Parent = root;
        ...
        private void LoadDB()
        {
            ...
            //GlobalService.ProjectFile = System.IO.Path.Combine(path, projectName + ".prj");
            var doc = new Document();
            ProjectType prj = null;
            if (!System.IO.File.Exists(GlobalService.ProjectFile))
            {
                var stream = System.IO.File.Create(GlobalService.ProjectFile);
                stream.Close();
            ...
            GlobalService.CurrentProjectDoc = doc;

            GlobalService.CurrentProject = prj;
            GlobalService.CurrentProject.Name = projectName;

            doc.Save(GlobalService.ProjectFile);
