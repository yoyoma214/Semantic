...
class People_School
{
	PeopleId(int,"int",false,true);
	SchoolId(int,"int",false,true);
}
...
//People.map(SchoolId,School,School,Id, ManyToMany,People_School,PeopleId,SchoolId);

People_School.map(PeopleId,People,School,SchoolId,ManyToMany,Schools,Peoples);

People.map(SchoolId,School,School,Id,ManyToOne,Name);