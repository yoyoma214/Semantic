        ...
        public override string RenderEFSpec(StringBuilder builder, string modelName,string condition_class, ParameterList parameters)
        {
            ...
            if (para.NullAble)
            {
                ...
                if (para.DbType == DbType.String)
                {
                    ...
                    builder.AppendLine(string.Format("\t\t\t  {0} =  new DirectSpecification<{1}>(x=> {2});", rslt, modelName, SelectContext.OrConext.FirstOrDefault() ? "false" : "true"));
                    builder.AppendLine("\t\t }");
                }
                else
                {
