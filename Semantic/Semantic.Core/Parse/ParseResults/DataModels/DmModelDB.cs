    ...
    public class DmModelDB : ParseModuleBase
    {
        ...
        public override void Wise()
        {
            ...
            foreach (var mapping in this.Mapings)
            {
                ...
                if (types != null && types.Count > 0)
                {
                    if (mapping.Relation.Value != "ManyToMany")
                    {
                        ...
                        var targetModelName = mapping.TargetModel.Value;
                        //if (mapping.Relation.Value == "ManyToMany")
                        //{
                        //    targetModelName = mapping
                        //}
        ...
        public void RenderSql(IndentStringBuilder builder)
        {
            ...
            foreach (var type in this.Types)
            {
                ...
                var pkFields = new List<FieldInfo>();
                foreach (FieldInfo f in type.PropertyInfos)
                {
                    if (f.Is_pk)
                    {
                        pkFields.Add(f);
...
", type.Name);
                    ...
                    for(var index = 0 ; index < pkFields.Count ; index ++ )
                    {
                        builder.AppendFormat(string.Format("	[{0}] ASC", pkFields[index].Name));
                        if (index < pkFields.Count - 1)
                            builder.Append(",");
                        builder.AppendLine();
                    }
                    builder.AppendFormatLine(@")WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
) ON [PRIMARY]
